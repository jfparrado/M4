...
describe("<Home />", () => {
  ...
  describe("React LifeCycles", () => {
    ...
    it("Debería pasar como props a cada componente <ProductCard /> las propiedades id, name, price , stock de cada product", () => {
      ...
      expect(home.find(ProductCard).at(1).props().name).toEqual(
        "Lapicera Henry"
      );
      ...
      expect(home.find(ProductCard).at(3).props().name).toEqual(
        "Zapatillas Henry"
      );
      ...
      expect(home.find(ProductCard).at(4).props().name).toEqual(
        "Pantalones Henry"
      );
